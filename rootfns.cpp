...
RootFns::RootFns(ConfGel *cgel) :Pointers(cgel) {}

void RootFns::lubksb(double **a, int n, int *indx, double *b)
{
  int i,ii=0,ip,j;
  double sum;
...
void RootFns::ludcmp(double **a, int n, int *indx, double &d)
{
...
void RootFns::mnewt(int ntrial, double *x, int n, 
		    double tolx, double tolf, double theta)
{
  int k,i;
...
void RootFns::fdjac(int n, double *x, double *fvec, double **df, double theta)
{
...
void RootFns::usrfun(double *F1, int n, double *fF1vec, double **fjac, double theta)
{
  
  confpg->calcfns(F1, fF1vec, theta);

  fdjac(n, F1, fF1vec, fjac, theta);

}


void RootFns::lnsrch(int n, double *xold, double fold, 
                     double *g, double *p, double *x, 
                     double &f, const double stpmax, bool &check,
                     double *fvec, double theta)
{
...
double RootFns::MAX(double a, double b) 
{
  return ( (a>b)? a:b);
}


void RootFns::newt(double *x, int n, bool &check, double theta)
{
  ...
  for (its=0;its<MAXITS;its++) {
    ...
    ludcmp(fjac,n,indx,d);
    lubksb(fjac,n,indx,p);
    lnsrch(n,xold,fold,g,p,x,f,stpmax,check,fvec,theta);
    test=0.0;
    for (i=0;i<n;i++)
      if (fabs(fvec[i]) > test) test=fabs(fvec[i]);
    if (test < TOLF) {
...
double RootFns::fmin(double *F1, double *fF1vec, int n, double theta)
{
  double sum;
  confpg->calcfns(F1, fF1vec, theta);
  sum = 0.0;
  for (int i=0;i<n;i++) sum += fF1vec[i]*fF1vec[i];
  return 0.5*sum;
}


void RootFns::qrupdt(double **r, double **qt, int n, double *u, double *v)
{
  int i,k;
...
void RootFns::qrdcmp(double **a, int n, double *c, double *d, bool &sing)
{
  int i,j,k;
...
void RootFns::rotate(double **r, double **qt, int n, int i, double a, double b)
{
  int j;
  double c,fact,s,w,y;
...
void RootFns::rsolv(double **a, int n, double *d, double *b)
{
  int i,j;
  double sum;
...
double RootFns::SIGN(double a, double b) 
{
  return ((b) >= 0.0 ? fabs(a) : -fabs(a));
}


void RootFns::broydn(double *x, int n, bool &check, double theta) 
{
  const int MAXITS=200;
